...
class RegularActor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, max_action, device_=None):
    ...
    def forward(self, state):
        ...
        z = mean_a + std_a * torch.FloatTensor(np.random.normal(0, 1, size=(std_a.size()))).to(self.device) 
    ...
    def sample_multiple(self, state, num_sample=10):
        ...
        z = mean_a.unsqueeze(1) +\
             std_a.unsqueeze(1) * torch.FloatTensor(np.random.normal(0, 1, size=(std_a.size(0), num_sample, std_a.size(1)))).to(self.device).clamp(-0.5, 0.5)
...
class VAE(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, latent_dim, max_action, device_=None):
    ...
    def decode_bc(self, state, z=None):
        if z is None:
                z = torch.FloatTensor(np.random.normal(0, 1, size=(state.size(0), self.latent_dim))).to(self.device)
    ...
    def decode_bc_test(self, state, z=None):
        if z is None:
                z = torch.FloatTensor(np.random.normal(0, 1, size=(state.size(0), self.latent_dim))).to(self.device).clamp(-0.25, 0.25)
...
class BEAR(object):
    def __init__(self, num_qs, state_dim, action_dim, max_action, delta_conf=0.1, use_bootstrap=True, version=0, lambda_=0.4,
        ...
        self.actor = RegularActor(state_dim, action_dim, max_action, device_=self.device).to(self.device)
        self.actor_target = RegularActor(state_dim, action_dim, max_action, device_=self.device).to(self.device)
        ...
        self.vae = VAE(state_dim, action_dim, latent_dim, max_action, device_=self.device).to(self.device)
        ...
        self.warmstart_epoch = warmstart_epoch
    ...
    def train_on_batch(self, batch, epoch):
        ...
        A copy of @train with a few changes:
            (1) Takes a single batch as input and trains on that single batch.
            (2) Returns training statistics to record.
            (3) Takes epoch number as external input to do warmstarting.
        """
        ...
        ret = {}
        ret["Lagrange2"] = self.log_lagrange2.exp().item()
        ret["Actor_Loss"] = actor_loss.item()
        ret["Critic_Loss"] = critic_loss.item()
        ret["Std_Loss"] = std_loss.mean().item()
        ret["MMD_Loss"] = mmd_loss.mean().item()
        ret["Action_Divergence"] = action_divergence.mean().item()
        ret["Raw_Action_Divergence"] = raw_action_divergence.mean().item()
        return ret
...
class BEAR_IS(object):
    ...
    def __init__(self, num_qs, state_dim, action_dim, max_action, delta_conf=0.1, use_bootstrap=True, version=0, lambda_=0.4, threshold=0.05,
                 ...
                 actor_lr=0.001, critic_lr=0.001, vae_lr=0.001, device_=None):
        self.device = device_
        if self.device is None:
            self.device = DEVICE
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        for it in tqdm(range(iterations)):
            ...
            mask            = torch.FloatTensor(mask).to(self.device)
            data_mean       = torch.FloatTensor(data_mean).to(self.device)
            data_cov        = torch.FloatTensor(data_cov).to(self.device)
...
class BCQ(object):
    def __init__(self, state_dim, action_dim, max_action, cloning=False, actor_lr=0.001, critic_lr=0.001, vae_lr=0.001, device_=None):
        self.device = device_
        if self.device is None:
            self.device = DEVICE
...
class DQfD(object):
    ...
    def __init__(self, state_dim, action_dim, max_action, lambda_=0.4, margin_threshold=1.0, device_=None):
        self.device = device_
        if self.device is None:
            self.device = DEVICE
        ...
        self.actor = ActorTD3(state_dim, action_dim, max_action).to(self.device)
        self.actor_target = ActorTD3(state_dim, action_dim, max_action).to(self.device)
        self.actor_target.load_state_dict(self.actor.state_dict())
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        ...
        for it in tqdm(range(iterations)):
            ...
            noise = torch.FloatTensor(action_np).data.normal_(0, policy_noise).to(self.device)
...
class KLControl(object):
    def __init__(self, num_qs, state_dim, action_dim, max_action, use_bootstrap=False, temperature=0.1, mode='auto', device_=None):
        self.device = device_
        if self.device is None:
            self.device = DEVICE
        ...
        self.vae = RegularActor(state_dim, action_dim, max_action, device_=self.device).to(self.device)
        self.vae_optimizer = torch.optim.Adam(self.vae.parameters())
        ...
        if self.mode == 'auto':
            self.temperature = torch.randn( (), requires_grad=True, device=self.device)
